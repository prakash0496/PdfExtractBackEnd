...
@Service
public class KvbBankStatementExcelService {

    public byte[] generateExcel(List<KvbTransactionDTO> transactions) throws Exception {
        try (Workbook workbook = new XSSFWorkbook()) {
            Sheet sheet = workbook.createSheet("Bank Statement");

            String[] headers = {"Transaction Date", "Value Date", "Branch", "Cheque No",
            ...
            for (KvbTransactionDTO tx : transactions) {
                ...
                row.createCell(2).setCellValue(tx.getBranch());
